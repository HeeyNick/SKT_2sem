...
void cube::init()
{
    ...
    for(size_t i = 0; i < 3; i++)
        for(size_t j = 0; j < 3; j++)
            ...
            {
                gauss_weights[/*i * j * k*/(i*3+j)*3+k] = gauss_weights_1d[i] * gauss_weights_1d[j] * gauss_weights_1d[k];
                gauss_points[/*i * j * k*/(i*3+j)*3+k] = point(gauss_points_1d[i], gauss_points_1d[j], gauss_points_1d[k]);
            }
    jacobian = mes / 8.0;
...
vector3 cube::get_B(const point & x) const
{
...
vector3 area::get_B(const point & x) const
{
    vector3 result(0, 0, 0);
...
double area::get_abs_B(const point & x) const
{
    return get_B(x).norm();
}
